...
\usepackage[colorlinks=true,linkcolor=black,citecolor=black,urlcolor=blue,
  ...
  pdfauthor={Muhammed Muiz}]{hyperref}
...
\author{Muhammed Muiz \\ \small Independent Researcher}
\date{August 2026\thanks{Code, configurations, and every reported number:
  \url{https://github.com/secslayer/jlens-fuzz}. Disclosure notices concerning the evaluated
  open-weight models were sent to their maintainers on 2026-08-19; see \cref{sec:ethics}.}}
...
did not run either as a baseline in this work}. A fixed compute budget (\cref{sec:setup}) scoped
the core comparison to GPTFuzzer alone, with AutoDAN deferred to an extended set of runs that the
budget did not reach. Separately, a growing body of work has begun questioning
...
\begin{itemize}
  \item \textbf{No successful jailbreak strings are published in this repository or paper.} Raw
    ...
    (\texttt{.github/workflows/ci.yml}, covering all branches, not only \texttt{main}), and is
    also available as a local pre-commit hook (\texttt{.githooks/pre-commit}) so violations are
    caught before they leave the machine. This is a real, meaningful control, not a perfect
    guarantee: it catches
  ...
  \item \textbf{Disclosure.} This project committed in advance, in writing and before any results
    existed, to notifying the affected open-weight model maintainers before the work was made
    public. That commitment was originally tied to arXiv posting as its trigger; it was
    superseded when the code repository was made public on 2026-08-17, a deliberate decision
    recorded at the time rather than after the fact.
