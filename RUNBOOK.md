...
> **Closing note (2026-08-09, what actually happened to Day 6/7):** `transfer_local`, `figures`,
> and `scripts/assemble_paper.py` above were never built or run — none of `scripts/`'s files
> match those names, and no `results/*transfer*` or `results/figures/` exists. `paper/paper.md`
> was instead assembled **by hand** directly from `results/*.json` (README.md confirms this), then
> converted to `paper/paper.tex` (the submission artifact) by hand as well, both numerically
> re-verified against the source JSONs rather than generated by a script. The **Release
> checklist** in PART 7 below reflects the real, current process — follow that, not the
> `assemble_paper.py` plan above.
